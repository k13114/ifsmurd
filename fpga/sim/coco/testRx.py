...
async def first_value_test(dut):
    ...
    await Timer(8700, units=unitsOfTime) # start bit
    dut.inputRx.value = 0b0
    await Timer(8700, units=unitsOfTime)
    dut.inputRx.value = 0b1
    await Timer(8700, units=unitsOfTime)
    dut.inputRx.value = 0b1
    await Timer(8700, units=unitsOfTime)
    dut.inputRx.value = 0b0
    ...
    await Timer(8700, units=unitsOfTime)
    dut.inputRx.value = 0b1
    await Timer(8700, units=unitsOfTime) # stop bit
...
def test_simple_dff_runner():
    ...
    hdlTopLevelInput = "uartRx"
    testModuleInput = "testRx"
    ...
    if hdl_toplevel_lang == "verilog":
        verilog_sources = [proj_path / "../verilog/uartRx.v",
                           ]
    else:
